...
SELECT 
   ...
   COUNT(*) as total_jobs,
   SUM(pj.credits_used) as credits_consumed,
   COUNT(DISTINCT pj.user_id) as unique_users,
   SUM(CASE WHEN u.subscription_tier = 'premium' THEN pj.credits_used ELSE 0 END) as premium_credits,
   SUM(CASE WHEN u.subscription_tier = 'pro' THEN pj.credits_used ELSE 0 END) as pro_credits,
   SUM(CASE WHEN u.subscription_tier = 'free' THEN pj.credits_used ELSE 0 END) as free_credits,
   AVG(pj.credits_used) as avg_credits_per_job
...
-- Model usage trends view
CREATE OR REPLACE VIEW model_usage_trends AS
SELECT 
...
FROM ai_models m
JOIN processing_jobs pj ON m.id = pj.model_id
WHERE pj.created_at >= NOW() - INTERVAL '30 days'
GROUP BY m.name, m.category, m.provider, DATE(pj.created_at)
...
SELECT 
   ...
   COUNT(CASE WHEN pj.created_at >= NOW() - INTERVAL '30 days' THEN 1 END) as jobs_last_30d,
   MAX(pj.created_at) as last_job_date,
   CASE 
       WHEN MAX(pj.created_at) >= NOW() - INTERVAL '7 days' THEN 'highly_active'
       WHEN MAX(pj.created_at) >= NOW() - INTERVAL '30 days' THEN 'active'
       WHEN MAX(pj.created_at) >= NOW() - INTERVAL '90 days' THEN 'inactive'
       ELSE 'dormant'
...
SELECT 
   ...
   AVG(processing_time_seconds) as avg_time_before_failure
FROM processing_jobs
