-- Analytics and logging tables for AI Image Enhancer
...
-- Create usage_analytics table for daily usage tracking
...
CREATE TABLE system_metrics (
    ...
    metric_type VARCHAR(50) DEFAULT 'gauge' CHECK (metric_type IN ('gauge', 'counter', 'histogram')),
    tags JSONB DEFAULT '{}',
...
-- Create file_storage table for tracking uploaded files
CREATE TABLE file_storage (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    job_id UUID REFERENCES processing_jobs(id) ON DELETE CASCADE,
    file_type VARCHAR(20) NOT NULL CHECK (file_type IN ('input', 'output', 'thumbnail')),
